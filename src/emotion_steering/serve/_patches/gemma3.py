...
# This is upstream vLLM 0.20.x gemma3.py with four surgical edits, marked
# `# STEERING:`. They:
#   1. Import steering hooks
#   2. Cache the layer index at construct time
#   3. At the chosen layer, add a per-token steering tensor to `residual` so it
#      fuses into the next layer's input_layernorm — matches the post-block
#      residual-stream injection site used during extraction.
#   4. Install steering wrapper from ForCausalLM.__init__ (covers multimodal wrapper)
...
from vllm._steering import get_steering_tensor, install as install_steering, is_steering_layer, record_projections
...
class Gemma3DecoderLayer(nn.Module):
    def __init__(
        ...
        try:
            ...
            self.layer_idx = -1
        # STEERING: idempotent install of GPUModelRunner wrapper
        install_steering()
    ...
    def forward(
        ...
        **kwargs,
        ...
        # `residual`. The addition lands in the post-block residual stream that
        # the next layer's input_layernorm will fuse with hidden_states — same
        # site captured during vector extraction.
...
class Gemma3ForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        # STEERING: idempotent install — covers the case where the multimodal
        # wrapper instantiates this without per-DecoderLayer init running first.
        install_steering()
        config = vllm_config.model_config.hf_config
        quant_config = vllm_config.quant_config
