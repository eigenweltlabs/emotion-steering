...
# This is upstream vLLM 0.20.x mistral.py with three STEERING edits:
#   1. Import steering hooks
#   2. Call install_steering() in MistralDecoderLayer.__init__ (idempotent)
#   3. In MistralDecoderLayer.forward, add per-token steering to `residual`
#      before return — lands in the post-block residual stream fused by
#      the next layer's input_layernorm. Matches the extraction capture site.
